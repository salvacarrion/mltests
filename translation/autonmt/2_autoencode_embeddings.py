...
def encode_data2(filename, enc_dim, name=""):
    # Save embeddings
    trg_emb = np.load(os.path.join(filename, "raw_glove.npy"))

    # # Standarized
    # trg_scaler = StandardScaler().fit(trg_emb)
    # trg_emb_scaled = trg_scaler.transform(trg_emb)
    trg_emb_scaled = trg_emb

    # Train autoencoder and encode data
    new_trg_emb, trg_emb_scaled_rec = ae_fit_transform(trg_emb_scaled, enc_dim=enc_dim)

    # Inverse scale
    trg_emb_rec = trg_emb_scaled_rec  #trg_scaler.inverse_transform(trg_emb_scaled_rec)
